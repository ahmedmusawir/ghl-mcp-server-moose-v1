...
class GHLMCPHttpServer {
    ...
    for (const tool of contactToolDefinitions) {
      this.mcpServer.registerTool(
        tool.name,
        {
          ...
          // REMOVE outputSchema - let MCP infer it from the response
          // outputSchema: { result: z.any() }
        },
        async (params: any) => {
          console.log(`[GHL MCP HTTP] Executing tool: ${tool.name}`);

          try {
            const result = await this.contactTools.executeTool(
              tool.name,
              params
            );

            console.log(
              `[GHL MCP HTTP] Tool result:`,
            ...
            return {
              ...
              structuredContent: result, // ADD THIS BACK!
            };
          } catch (error) {
            console.error(
              `[GHL MCP HTTP] Error executing tool ${tool.name}:`,
              error
            );
  ...
  private setupRoutes(): void {
    ...
    // STREAMABLE HTTP ENDPOINT - This is the key change!
